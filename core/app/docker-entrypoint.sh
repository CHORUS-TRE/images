...
if [ "$(id -u)" = "0" ]; then
    ...
    exec runuser -l "$CHORUS_USER" -w DISPLAY,LD_PRELOAD,NSS_WRAPPER_PASSWD,NSS_WRAPPER_GROUP,BROWSER_URL,BROWSER_JWT_URL,BROWSER_JWT_TOKEN,APP_NAME,CARD -c "$CMD"
else
