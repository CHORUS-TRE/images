...
if [ "$(id -u)" = "0" ]; then
    ...
    exec runuser -l "$CHORUS_USER" -w DISPLAY,LD_PRELOAD,NSS_WRAPPER_PASSWD,NSS_WRAPPER_GROUP,BROWSER_URL,IDP_JWT_URL,IDP_SL_TOKEN,IDP_LL_TOKEN,APP_NAME,CARD -c "$CMD"
else
